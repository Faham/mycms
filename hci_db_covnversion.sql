...
CHANGE `Affiliation`     `people_affiliation` varchar(100) COLLATE utf8_unicode_ci DEFAULT NULL  AFTER `people_lastname`,
CHANGE `Email`           `people_email`       varchar(50) COLLATE utf8_unicode_ci DEFAULT NULL   AFTER `people_affiliation`,
CHANGE `Bio`             `people_bio`         varchar(3000) COLLATE utf8_unicode_ci DEFAULT NULL AFTER `people_email`,
...
CHANGE  `Abstract`              `publication_abstract`           varchar(4000) COLLATE utf8_unicode_ci NOT NULL AFTER `publication_booktitle`,
...
CHANGE  `DOI`                   `publication_doi_number`         varchar(50)  COLLATE utf8_unicode_ci DEFAULT NULL AFTER `publication_pages`,
...
CHANGE  `ISBN`                  `publication_isbn`               varchar(20)  COLLATE utf8_unicode_ci DEFAULT NULL AFTER `publication_journal`,
...
CHANGE  `TagLine`  `research_summary`     varchar(300) COLLATE utf8_unicode_ci DEFAULT NULL  AFTER `research_title`,
CHANGE  `Summary`  `research_description` varchar(1500) COLLATE utf8_unicode_ci DEFAULT NULL AFTER `research_summary`,
...
-- select * from `mycms_image_people` wehre `people_id` not in (select `people_id` from `mycms_people`);
...
INSERT `mycms_image` (`image_filename`) SELECT CONCAT('people-', `people_id`, '.jpg') FROM `mycms_image_people`;
UPDATE `mycms_image_people` as a
	JOIN `mycms_image` as b
	ON b.`image_filename` = CONCAT('people-', a.`people_id`, '.jpg')
...
INSERT `mycms_image` (`image_filename`) SELECT CONCAT('research-', `research_id`, '.jpg') FROM `mycms_image_research`;
UPDATE `mycms_image_research` as a
	JOIN `mycms_image` as b
	ON b.`image_filename` = CONCAT('research-', a.`research_id`, '.jpg')
	SET a.`image_id` = b.`image_id`;
...
INSERT `mycms_image` (`image_filename`) SELECT CONCAT('publication-', `publication_id`, '.jpg') FROM `mycms_image_publication`;
UPDATE `mycms_image_publication` as a
	JOIN `mycms_image` as b
	ON b.`image_filename` = CONCAT('publication-', a.`publication_id`, '.jpg')
	SET a.`image_id` = b.`image_id`;
